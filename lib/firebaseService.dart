...
class FirebaseService {
  ...
  Future<Color> getQuoteFontColor() async {
    final prefs = await SharedPreferences.getInstance();
    final colorValue = prefs.getInt('fontColor');
    final color = colorValue != null ? Color(colorValue) : Colors.white;
    print("FirebaseService loaded font color: $color - $colorValue");
    return Color(prefs.getInt('fontColor') ?? Colors.white.value);
  }

  Future<double> getQuoteFontSize() async {
    final prefs = await SharedPreferences.getInstance();
    return prefs.getDouble('fontSize') ?? 22.0;
  }
