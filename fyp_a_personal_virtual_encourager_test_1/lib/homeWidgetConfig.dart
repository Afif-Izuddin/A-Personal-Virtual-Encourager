...
class _HomeWidgetConfigScreenState extends State<HomeWidgetConfigScreen> {
  ...
  Future<void> _saveRefreshFrequency(String? newValue) async {
    if (newValue != null) {
      ...
      _startBackgroundUpdates(); // Start or update background updates based on new frequency
    }
  }
  ...
  void _startBackgroundUpdates() {
    Workmanager().cancelByUniqueName('daily_quote_refresh'); // Cancel any existing task

    Duration interval;
    switch (_refreshFrequency) {
      case 'Every 15 minutes': // Handle 'Every minute'
    ...
    Workmanager().registerPeriodicTask(
      'daily_quote_refresh', // A unique name for the task
      'fetchDailyQuoteAndUpdateWidget', // The task name that will be executed in the background
      frequency: interval,
    );
  }
